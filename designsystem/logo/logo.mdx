import { Meta, Canvas } from '@storybook/blocks';
import * as stories from './logo.stories';

<Meta of={stories} />

# Logo

> ⚠️ **OBS:** Logojustering kommer for å bedre støtte applikasjoner

- Legg klassen `logo` på typisk `<h1>` eller `<a>`
- Legg inn navn på din applikasjon som undermerke
- Usynlig prefiks-tekst "Mattilsynet" blir automatisk lagt inn for skjermlesere
- Bruk `&nbsp;` istedenfor mellomrom, eksempel: `Min&nbsp;admin`
- Logo følger tekststørrelse og tekstfarge (burde være `#054449`)
- Ved lite plass blir "Mattilsynet" skjult visuelt, og deretter undermerke:
- **Merk:** Logo blir engelsk dersom du setter `<html lang="en">`
<Canvas of={stories.Default} />

## Størrelser
- Logo støtter `data-size="md | lg"`
<Canvas of={stories.Sizes} />

## Engelsk
- Legg sett `lang="en"` på `<html>` eller elementet du bruker til logo
- Ikke bruk `&nbsp;` i engelske logoer
- **Merk:** Skjuler aldri "Mattilsynet, Norwegian Food Safety Authority" fordi dette er viktig i internsasjonal sammenheng
<Canvas of={stories.English} />

## Favicon
- For å støtte dark mode og eldre Safari, bruker vi flere favicon-filer (<a href="/favicon.svg" download>svg</a>, <a href="/favicon.ico" download>ico</a> og <a href="/favicon-dark.ico" download>ico dark</a>)
- Disse er tilgjengelig via `@mattilsynet/design/favicon.[svg|ico]`
```html
<head>
  <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
  <link rel="icon" href="/favicon.ico" media="(prefers-color-scheme: light)" />
  <link rel="icon" href="/favicon-dark.ico" media="(prefers-color-scheme: dark)" />
</head>
```