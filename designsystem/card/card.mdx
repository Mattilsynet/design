...
- **Fluid padding:** Bruk `data-pad="TALL-TALL"` - da får du voksende padding fra første tall `375px` til siste tall på `1440px`
...
## Border radius
- Både `card` og `group` støtter `data-radius="sm | md | lg | xl"` for endring av padding (`lg` er default for card, og `xl` er default for group)
