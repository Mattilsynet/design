...
# <ArrowUpRightIcon /> Migrering til versjon 3

> [Designsystemet](https://www.designsystemet.no/) tilbyr nå [rammeverkuavhengige komponenter](https://designsystemet.no/no/blog/designsystemet-web-pre2).
> Mattilsynet faser derfor ut egen kode til fordel for ut-av-boksen-løsninger.
> Det er ingen breaking changes, men heller anbefalte oppdateringer du kan gjøre etterhvert som de dukker opp i nettleserens konsoll.

<JumpTo />

## Konsoll-varsler

`@mattilsynet/design` gir automatisk advarser om bruk av utdaterte klasser og elementer.
Du kan skru av konsoll-varsler, for eksempel i produksjon eller test-miljø:
...
<Alert>**Merk:** Du kan få en advarsel `"Fields should only have one input element. Use <fieldset> to group multiple fields"` på Suggestion. Denne er bare å overse, og vil forsvinne ved neste versjon av Designsystemet.</Alert>

<br />
## Endringer ikke-React

`@mattilsynet/design` inneholder kode for automatisk bakoverkompatbilitet, men du burde over tid oppdatere følgende:
...
<Table data-size="sm">
  ...
  <Table.Tbody>
    ...
    <Table.Tr>
      ...
      <Table.Td>➡️ `class="styles.togglegroup" aria-label="LABEL" focusgroup="radiogroup"` Klasse og attributt</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Th>Togglegroup knapper</Table.Th>
      <Table.Td>❌ `<label>` Kun element</Table.Td>
      <Table.Td>➡️ `<label class="styles.button">` Element og klasse</Table.Td>
    </Table.Tr>
    <Table.Tr>
...
Som React-bruker får du flere migeringer under panseret automatisk av React-komponentene.
`@mattilsynet/design` inneholder kode for automatisk bakoverkompatbilitet, men du burde over tid oppdatere følgende:
