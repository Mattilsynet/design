...
- Field beriker indre skjemaelementer med f.eks automatisk sammenkobling av `<label>` og `<input>`, beskrivelser (`<p>`) og valideringer `validation`, samt `textarea` skalering, `textarea` antall tegn, `datalist` posisjonering etc.
- **Merk:** Field skal kun inneholde ét skjemaelementet, men du kan knytte flere sammen (f.eks fornavn + etternavn osv.) med [Fieldset](/?path=/docs/designsystem-fieldset--docs)
...
- **Merk:** `u-combobox` krever `input aria-required="true"` (ikke `input required`) siden combobox ikke er et native skjemaelement
- **Merk:** Fieldset støtter også `data-validation="form"` for grupper felt
...
- **Merk:** Combobox må alltid ha en tilknyttet `<label>`, men denne kan skjules med `hidden`
