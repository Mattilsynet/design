...
function toggleExpanded (toggle = true) {
  ...
  CSS_SHEET?.replaceSync(`:root { ${KEY_TOGGLE}: var(${KEY_TOGGLE}--${next}) }`);
...
function handleScroll() {
  ...
  if (STICK_DIR !== -1 && (NAV_H <= WIN_H || SCROLL_UP && WIN_Y <= NAV_Y)) { // Not sticking to top and scrolling down or sidebar is smaller than viewport
    ...
    STICK_DIR = -1;
    console
  } else if (STICK_DIR === -1 && !SCROLL_UP) { // Sticking to top and scrolling down 
    NAV.style.setProperty(KEY_NAV_POS, 'relative');
    NAV.style.setProperty(KEY_NAV_TOP, `${Math.max(0, WIN_Y - OFFSET_Y)}px`);
    NAV.style.removeProperty(KEY_NAV_BOTTOM);
    NAV.style.setProperty(KEY_NAV_ALIGN, "start");
    STICK_DIR = 0;
  } else if (STICK_DIR !== 1 && !SCROLL_UP && (WIN_Y + WIN_H) >= (NAV_Y + NAV_H)) { // Not sticking to bottom and scrolling down
    NAV.style.setProperty(KEY_NAV_POS, 'sticky');
    ...
    NAV.style.setProperty(KEY_NAV_POS, 'relative');
    NAV.style.setProperty(KEY_NAV_TOP, `${Math.min(NAV_Y - OFFSET_Y, WIN_Y + WIN_H - NAV_H - OFFSET_Y)}px`);
    NAV.style.removeProperty(KEY_NAV_BOTTOM);
    NAV.style.setProperty(KEY_NAV_ALIGN, "start");
    STICK_DIR = 0;
  }
